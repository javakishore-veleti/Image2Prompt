"""OpenRouter provider — delegates to the img2pmpt-caf-routers library (the single
source of LLM routing). The controller just adapts Req/Resp shapes."""
...
class OpenRouterController(ProviderController):
    key = "openrouter"
    implemented = True
    ...
    def _run_sync(self, req: ProviderInvokeReq) -> tuple[str, dict]:
        ...
        if not resp.success:
            raise RuntimeError(resp.error_message or resp.error_code or "openrouter failed")
        return resp.output_text, {"provider": "openrouter", "model": resp.model, **resp.raw}

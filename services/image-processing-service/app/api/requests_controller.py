from __future__ import annotations

import json

from fastapi import APIRouter, Depends, File, Form, HTTPException, Query, UploadFile
from sqlalchemy.orm import Session

from image2prompt_shared.api_errors import ensure_ok
from image2prompt_shared.auth_dep import Principal

from ..deps import current_customer, get_db
from ..di import get_image_facade
from ..dtos.internal_dtos import GetRequestReq, ListRequestsReq, ProcessImageReq
from ..facades.interfaces import IImageFacade
from ..schemas import ProcReqOut

router = APIRouter(prefix="/requests", tags=["requests"])

DEFAULT_INSTRUCTION = "Generate a detailed text-to-image prompt that could recreate this image."


def _parse_providers(providers: str | None) -> list[str] | None:
    if not providers:
        return None
    providers = providers.strip()
    try:
        parsed = json.loads(providers)
        if isinstance(parsed, list):
            return [str(p) for p in parsed]
    except json.JSONDecodeError:
        pass
    return [p.strip() for p in providers.split(",") if p.strip()]


@router.post("", response_model=ProcReqOut, status_code=201)
async def create_request(
    image: UploadFile = File(...),
    instruction: str = Form(default=DEFAULT_INSTRUCTION),
    project_id: str | None = Form(default=None),
    providers: str | None = Form(default=None),
    principal: Principal = Depends(current_customer),
    db: Session = Depends(get_db),
    facade: IImageFacade = Depends(get_image_facade),
):
    data = await image.read()
    if not data:
        raise HTTPException(status_code=400, detail="Empty image upload")
    resp = ensure_ok(
        await facade.process_image(
            ProcessImageReq(
                db=db,
                customer_id=principal.id,
                image_bytes=data,
                content_type=image.content_type or "image/png",
                filename=image.filename or "upload.png",
                instruction=instruction,
                project_id=project_id,
                requested_providers=_parse_providers(providers),
            )
        )
    )
    return resp.request


@router.get("", response_model=list[ProcReqOut])
def list_requests(
    principal: Principal = Depends(current_customer),
    db: Session = Depends(get_db),
    facade: IImageFacade = Depends(get_image_facade),
    limit: int = Query(default=50, le=200),
    offset: int = Query(default=0, ge=0),
):
    resp = ensure_ok(
        facade.list_requests(
            ListRequestsReq(db=db, customer_id=principal.id, limit=limit, offset=offset)
        )
    )
    return resp.requests


@router.get("/{request_id}", response_model=ProcReqOut)
def get_request(
    request_id: str,
    principal: Principal = Depends(current_customer),
    db: Session = Depends(get_db),
    facade: IImageFacade = Depends(get_image_facade),
):
    resp = ensure_ok(
        facade.get_request(GetRequestReq(db=db, customer_id=principal.id, request_id=request_id))
    )
    return resp.request

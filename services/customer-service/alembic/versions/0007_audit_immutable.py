...
Installs a trigger that rejects UPDATE/DELETE on audit_log. No-op on SQLite.

Revision ID: 0007_audit_immutable
...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE OR REPLACE FUNCTION img2pmpt_cust_audit_immutable()
        RETURNS trigger AS $$
        BEGIN
    ...
    op.execute(
        ...
        f"FOR EACH ROW EXECUTE FUNCTION img2pmpt_cust_audit_immutable();"
    )
    op.execute(
...
def downgrade() -> None:
    ...
    op.execute(f"DROP TRIGGER IF EXISTS audit_no_update ON {tbl};")
    op.execute(f"DROP TRIGGER IF EXISTS audit_no_delete ON {tbl};")
    op.execute("DROP FUNCTION IF EXISTS img2pmpt_cust_audit_immutable();")
